...
$$\operatorname*{argmin}_a \frac{a^T(\bar {\mathbf X}_1 - \bar {\mathbf X}_2)(\bar {\mathbf X}_1 - \bar {\mathbf X}_2)^Ta}{a^T \boldsymbol \Sigma a}$$
...
$$D^2(X_0, g) = \sum_{k=1}^{rank(B)} [Y_0^{(k)} - \bar Y_{g.}^{(k)}]^2$$
- Allocate $X_0$ to 
$$\operatorname*{argmin}_g D^2(X_0, g)$$
